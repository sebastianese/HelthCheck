...
$users = " <YourEmail@YourDomain.com>" # List of users to email your report to (separate by comma)
$fromemail = "Health Check <HealthCheck@YourDomain.com>"
...
foreach ($computer in $computers) {
    ...
    # Region Pings
    $PingsReport = @()
    $ping = Test-Connection -ComputerName www.google.com -Count 1
    foreach ($pingob in $ping) {
		$row = New-Object -Type PSObject -Property @{
			PSComputerName = $pingob.PSComputerName
			Address = $pingob.Address
			IPV4Address = $pingob.IPV4Address
			BufferSize = $pingob.BufferSize
		}
		$PingsReport += $row
    }
    $PingsReport = $PingsReport | ConvertTo-Html -Fragment
	# endregion

    # Region NsLookup
    $NsLookupReport = @()
    $lookup = nslookup www.google.com 
    foreach ($lookupob in $lookup) {
		$row = New-Object -Type PSObject -Property @{
			Server = $lookupob.Server
			Address = $lookupob.Address
            Name = $lookupob.Name
            Addresses = $lookupob.Addresses
			}
		$NsLookupReport += $row
    }
    $NsLookupReport = $lookup | ConvertTo-Html -Fragment

	# endregion
# Region Check Reboot
    $RebootReport = @()
    $reboot = get-winevent -FilterHashtable @{Logname='system';ID=1074}  -MaxEvents 3 
    foreach ($rebootob in $reboot) {
		$row = New-Object -Type PSObject -Property @{
			Time = $rebootob.TimeCreated
            Name = $rebootob.ProviderName
			Message = $rebootob.Message
			
		}
		$RebootReport += $row
    }
    $RebootReport = $RebootReport | ConvertTo-Html -Fragment
	# endregion
    ...
    <h3>Connectivity Test - Testing Internet Connectivity and Default Gateway</h3>
	<p>Results for Ping Test To: www.google.com for $computer</p>
	<table class="normal">$PingsReport</table>
	</div>

    <h3>Name Server Lookup Test - Testing Connectivity with Domain Controller (If Joined to a Domain)</h3>
	<p>NSlookup for www.google.com on $computer (If server is connected to a domain the resoponse will originate on the Domain Controller)</p>
	<table class="normal">$lookup</table>
	</div>

    <h3>Last Reboot report - Checking last 3 Reboot events (1074)</h3>
	<p>Last 3 Reboots for $computer (System Event ID 1074)</p>
	<table class="normal">$RebootReport</table>
	</div>
...
send-mailmessage -from $fromemail -to $users -subject "Automated System Report for $computers" -Attachments $ListOfAttachments -BodyAsHTML -body $HTMLmessage -UseSsl -priority Normal -smtpServer $server 
